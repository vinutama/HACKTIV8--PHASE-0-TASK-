...
function shoppingTime(memberId, money) {
    if (memberId === '' || memberId === undefined) {
        ...
        var barangHarga = [['Sepatu Stacattu', 1500000], ['Baju Zoro', 500000], ['Baju H&N', 250000], ['Sweater Uniklooh', 175000], ['Casing HP', 50000]];
        var result = {
        ...
        for (var i = 0; i < barangHarga.length; i++) {
            if (saldo >= barangHarga[i][1]) {
                listBelanja.push(barangHarga[i][0]);
                saldo = saldo - barangHarga[i][1];
            }
        } result.listPurchased = listBelanja;
        result.changeMoney = saldo;
